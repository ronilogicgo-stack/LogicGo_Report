...
export default function AnnualReportAccessPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: teamData }, { data: grantData }] = await Promise.all([
      supabase
        ...
        .order("full_name"),
      supabase.from("annual_report_access").select("*, profiles:user_id(full_name, email)"),
    ]);
    setTeam(teamData || []);
    setGrants(grantData || []);
    setLoading(false);
  ...
  async function removeGrant(id) {
    if (!confirm("Remove this person's access to the Annual Report?")) return;
    await supabase.from("annual_report_access").delete().eq("id", id);
    load();
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <h1 className="text-lg sm:text-xl font-bold">Annual Report · Team Access</h1>
      <p className="text-sm text-slate-500">
        Grant a team member Viewer (read-only), Editor (can also edit Remarks),
        or Agency Owner (Editor, plus can manage this list themselves) access
        to the Annual Report. This is separate from Sales Person / Admin roles.
      </p>

      {!checkingAccess && !canManage && (
        <p className="text-sm bg-amber-50 text-amber-700 border border-amber-200 rounded-lg px-4 py-3">
          You don't have permission to grant or remove Annual Report access.
          Only the account owner ({OWNER_EMAIL}) or someone made an Agency
          Owner of this report can do that. You can view the current list
          below.
        </p>
      )}
      ...
      {loading ? (
        ...
        <p className="text-slate-500">No access granted yet - only the Owner can see the Annual Report.</p>
      ) : (
