...
/**
 * Payment Follow-Up priority - matches the logic from the original
 * Google Sheet's Apps Script: among the 5 possible follow-up dates,
 * find the latest one. If the payment is still Due and that latest
 * date has arrived or passed, it's "red" (urgent). If it's tomorrow,
 * it's "yellow" (upcoming). Otherwise it's "normal". Received records
 * are always "normal" regardless of follow-up dates.
 */
export function followupPriority(record) {
  if (record.payment_status === "Received") return "normal";
  ...
  if (dates.length === 0) return "normal";

  const latest = dates.reduce((max, d) => (d > max ? d : max), dates[0]);
  const today = dateKey();
  const tomorrow = dateKey(new Date(Date.now() + 24 * 60 * 60 * 1000));

  if (latest <= today) return "red";
  if (latest === tomorrow) return "yellow";
  return "normal";
}

/**
 * Sorts Payment Follow-Up records the same way the original Apps
 * Script physically reordered spreadsheet rows: red first, then
 * yellow, then everything else by serial number ascending. Here it's
 * just a stateless sort at query time - no locking or reordering of
 * stored data is needed.
 */
export function sortFollowups(records) {
  const rank = { red: 0, yellow: 1, normal: 2 };
